...
// Your JS code here
    ...
    getDOMSnapshot(element, depth = 0) {
        ...
        if (['script', 'style', 'svg', 'omni-agent'].includes(tag)) return `<${tag}>...</${tag}>`;
            ...
            if (profile.provider === 'openai') {
                ...
                const res = await fetch('https://api.anthropic.com/v1/models', {
                    headers: {
                        ...
                        'anthropic-dangerous-direct-browser-access': 'true' // Crucial for client-side CORS
                    }
                });
                const data = await res.json();
                if (data.error) throw new Error(data.error.message);
    ...
    async callLLM(history) {
        ...
        if (p.provider === 'openai') {
            ...
            // Normalize history: Anthropic requires strictly alternating roles
            const normalized = [];
    ...
    appendMessage(role, content, isHtml = false) {
        ...
        if (isHtml) {
            ...
            // Simple markdown parsing for code and newlines
