class OmniAgent extends HTMLElement {
    constructor() {
        ...
        // 2. Cloud Sync Vault (GitHub Gist)
        this.GIST_ID = 'd8f5a8cc13089257adc2f03dd373530c';
        this.ENCRYPTED_PAT = '4kCCaVV0b1ds6pefD8y6DCmEcFVLpMKaRqcdqY41jjpK4QsXAi8JPoTVL7Cae0/lNZYW4KL78j+GyR54R2/tHAMBVGisERsGGNitwBqBDU1i/Moe';
    ...
    async unlockCrypto(password) {
        try {
            ...
            const combined = new Uint8Array(atob(this.ENCRYPTED_PAT).split("").map(c => c.charCodeAt(0)));
            ...
            // Derive a key we will keep in memory for both decrypting the PAT AND encrypting the Vault Data
            ...
            this.pat = new TextDecoder().decode(decrypted);
        } catch (error) {
            throw new Error("Incorrect Master Password or corrupted payload.");
        }
    }
    ...
    async decryptVaultData(encryptedString) {
        try {
            ...
            // Failsafe: if the gist was somehow saved unencrypted previously
    ...
    async updateVault(profilesArray) {
        if (!this.pat || !this.isSynced || !this.vaultKey) return;
    ...
    async handleUnlockVault(password) {
        ...
        try {
            // 1. Generate keys and unlock PAT
...
7. Keep your conversational responses concise.`;
    ...
    render() {
        let template = `
            <style>
                ...
                .btn-outline { background: transparent; border: 1px solid var(--border); color: var(--text); font-weight: 600; padding: 12px; border-radius: 8px; cursor: pointer; width: 100%; }
                ...
                /* Vault UI */
                .vault-box { background: #1e293b; border: 1px solid #334155; padding: 16px; border-radius: 8px; margin-bottom: 16px;}
                .vault-box label { color: #94a3b8; }
                .vault-box input { background: #0f172a; color: white; border: 1px solid #475569; }
                .vault-title { color: #38bdf8; margin: 0 0 12px 0; font-size: 14px; display: flex; align-items: center; gap: 8px;}
            ...
            <div id="panel">
                ...
                <div id="settings-view" class="view">
                    ...
                    <!-- Cloud Sync Vault UI -->
                    <div class="vault-box">
                        <h3 class="vault-title">☁️ Cloud Vault Sync</h3>
                        
                        <div id="vault-locked-ui">
                            <div class="form-group">
                                <label>Master Password</label>
                                <input type="password" id="vault-password" placeholder="Enter password to sync">
                            </div>
                            <button class="btn-block" style="background:#38bdf8; color:#0f172a;" id="btn-unlock-vault">Unlock & Sync</button>
                        </div>

                        <div id="vault-unlocked-ui" style="display:none;">
                            <p style="color:#10b981; font-weight:bold; margin-top:0; font-size:13px;">✅ Vault Unlocked & Synced</p>
                            <button class="btn-outline" style="border-color:#ef4444; color:#ef4444;" id="btn-lock-vault">Lock Vault</button>
                        </div>
    ...
    renderProfilesList() {
        ...
        if (this.profiles.length === 0) {
            list.innerHTML = '<p style="font-size:12px; color:var(--text-muted);">No profiles found. Add one below or Unlock Vault to sync.</p>';
            return;
        }
