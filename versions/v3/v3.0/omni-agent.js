...
7. Keep your conversational responses concise.`;
    ...
    async fetchModelsForProfile(profile) {
        ...
        try {
            ...
            else if (profile.provider === 'aihorde') {
                const targetUrl = 'https://aihorde.net/api/v2/status/models?type=text';
                const res = await fetch(this.proxyUrl + encodeURIComponent(targetUrl));
                const data = await res.json();
                models = data.map(m => m.name).sort();
            }
    ...
    getFallbackModels(provider) {
        ...
        if (provider === 'aihorde') return ['aphrodite/TheDrummer/Behemoth-R1-123B-v2-w4a16', 'aphrodite/TheDrummer/Cydonia-24B-v4.3', 'koboldcpp/L3-8B-Stheno-v3.2.i1-Q4_K_M', 'koboldcpp/gemma-4-26B-A4B-it'];
        return [];
    }
    ...
    async callLLM(history) {
        ...
        if (p.provider === 'openai') {
        ...
        else if (p.provider === 'aihorde') {
            const targetUrl = 'https://aihorde.net/api/v2/generate/text/async';
            let promptText = sysPrompt + "\n\n";
            for (let m of history) {
                promptText += `${m.role === 'user' ? 'User' : 'Assistant'}: ${m.content}\n`;
            }
            promptText += "Assistant:";

            let finalMaxContext = p.maxContext || 4096;
            let finalMaxGen = p.maxGen || 512;
            const modelsArr = p.model ? p.model.split(',') : [];

            if (p.smartRouting !== false) {
                try {
                    const wRes = await fetch(this.proxyUrl + encodeURIComponent('https://aihorde.net/api/v2/workers?type=text'));
                    const workers = await wRes.json();
                    
                    let bestContext = 0;
                    let bestGen = 0;
                    let hasMatchingWorker = false;
                    
                    for (const w of workers) {
                        if (!w.online || w.paused || w.maintenance_mode) continue;
                        const servesModel = modelsArr.length === 0 || modelsArr.some(m => w.models.includes(m));
                        if (servesModel) {
                            hasMatchingWorker = true;
                            if (w.max_context_length > bestContext) bestContext = w.max_context_length;
                            if (w.max_length > bestGen) bestGen = w.max_length;
                        }
                    }
                    
                    if (!hasMatchingWorker) {
                        if (!confirm("Warning: No active workers are currently serving your selected model(s). Your request might get stuck waiting. Proceed anyway?")) {
                            throw new Error("Request cancelled by user (No active workers).");
                        }
                    } else {
                        // Calculate a smart safe limit based on prompt size + best available gen
                        const approxPromptTokens = Math.ceil(promptText.length / 3.5);
                        finalMaxGen = Math.min(finalMaxGen, bestGen); 
                        finalMaxContext = Math.min(Math.max(approxPromptTokens + finalMaxGen, 1024), bestContext);
                        console.log(`🤖 Smart Routing: Adjusted to Context=${finalMaxContext}, Gen=${finalMaxGen}`);
                    }
                } catch (e) {
                    if (e.message.includes("cancelled")) throw e;
                    console.warn("Smart Routing failed to fetch workers:", e);
                }
            }
            ...
            const payload = {
                prompt: promptText,
                params: {
                    max_context_length: finalMaxContext,
                    max_length: finalMaxGen,
                    temperature: 0.75,
                    top_p: 0.92,
                    rep_pen: 1.05,
                    stop_sequence: ["User:", "\nUser"]
            ...
            reqHeaders['apikey'] = p.apiKey && p.apiKey.trim() !== '' ? p.apiKey : '0000000000';
            ...
            let attempts = 0;
            let promptedImpossible = false;
            
            while (attempts < 60) { // 2 minutes max
                await new Promise(r => setTimeout(r, 2000));
                const pollRes = await fetch(this.proxyUrl + encodeURIComponent(statusUrl), { cache: 'no-store' });
                const pollData = await pollRes.json();
                
                if (pollData.faulted) throw new Error("AI Horde text generation failed.");
                if (pollData.done) return pollData.generations[0].text.trim();
                
                if (pollData.is_possible === false && !promptedImpossible) {
                    promptedImpossible = true;
                    if (!confirm("The Horde reports no workers can fulfill this request (is_possible: false). Do you want to keep waiting? (Cancel to abort)")) {
                        fetch(this.proxyUrl + encodeURIComponent(statusUrl), { method: 'DELETE', headers: reqHeaders }).catch(() => {});
                        throw new Error("Request cancelled by user (Impossible request).");
                    }
                }
                attempts++;
            }
                ...
                <div id="settings-view" class="view">
                    ...
                    <div id="settings-tab-profiles" class="tab-pane active">
                        ...
                        <div class="fancy-input-group">
                            ...
                            <select class="fancy-input" id="set-provider">
                                ...
                                <option value="aihorde">AI Horde (Text Gen)</option>
                            </select>
                        </div>
                        <div class="fancy-input-group">
                        ...
                        <div class="fancy-input-group">
                            <span class="fancy-icon">🔑</span>
                            <input type="password" class="fancy-input" id="set-key" placeholder="API Key (Optional for AI Horde)">
                            <a id="api-link" style="font-size:11px;color:#6366f1;text-decoration:none;margin-top:4px;display:block;" href="https://platform.openai.com/api-keys" target="_blank">Get an API Key</a>
                        </div>
                    ...
                    <div id="settings-tab-models" class="tab-pane">
                        ...
                        <div id="model-config-area">
                            ...
                            <div id="aihorde-settings" style="display:none; padding: 12px; background: #f8fafc; border: 1px solid #e2e8f0; border-radius: 10px; margin-bottom: 20px;">
                                <h4 style="margin:0 0 10px 0; font-size:12px; color:#0f172a;">AI Horde Generation Limits</h4>
                                
                                <label style="display:flex; align-items:center; gap:8px; margin-bottom:12px; cursor:pointer;">
                                    <input type="checkbox" id="horde-smart-routing" checked>
                                    <span style="font-weight:600; font-size:13px; color:#0f172a;">Smart Parameters & Routing</span>
                                </label>
                                <p style="font-size:10px; color:#64748b; margin:0 0 12px 0; line-height:1.4;">Automatically checks active workers to match your request to their exact capabilities, ensuring it can be fulfilled.</p>

                                <div id="horde-manual-settings" style="display:none;">
                                    <label style="margin-bottom:4px;">Max Context Length (Tokens)</label>
                                    <input type="number" class="fancy-input" id="horde-context-len" value="4096" style="padding-left:12px; margin-bottom:10px;" min="1024" max="128000">
                                    <label style="margin-bottom:4px;">Max Generation Length (Tokens)</label>
                                    <input type="number" class="fancy-input" id="horde-gen-len" value="512" style="padding-left:12px; margin-bottom:8px;" min="64" max="4096">
                                </div>
                            </div>

                            <button class="btn-primary" id="btn-update-model">Apply Model</button>
                        </div>
                        <div id="model-config-empty" style="display:none; text-align:center; padding: 20px; color: #64748b; font-size: 13px;">
                            Please create and select a profile in the Profiles tab first.
                        </div>
                    </div>
    ...
    bindEvents() {
        ...
        const providerSelect = sr.getElementById('set-provider');
        providerSelect.addEventListener('change', (e) => {
            const link = sr.getElementById('api-link');
            const keyInput = sr.getElementById('set-key');
            ...
            else if (e.target.value === 'aihorde') { link.href = 'https://aihorde.net/register'; link.innerText = 'Get a free AI Horde API Key'; }
            else { link.href = 'https://aistudio.google.com/app/apikey'; link.innerText = 'Get a Google Gemini API Key'; }
            
            if (e.target.value === 'aihorde') {
                keyInput.placeholder = "API Key (Optional for AI Horde)";
            } else {
        ...
        sr.getElementById('btn-save-profile').addEventListener('click', async () => {
            ...
            if (!key && provider !== 'aihorde') return alert("API Key is required.");
        ...
        sr.getElementById('btn-update-model').addEventListener('click', async () => {
            ...
            const selectEl = sr.getElementById('set-model');
            
            let selectedModels;
            if (active.provider === 'aihorde') {
                selectedModels = Array.from(selectEl.selectedOptions).map(opt => opt.value).join(',');
            } else {
                selectedModels = selectEl.value;
            }
            
            active.model = customModel || selectedModels;
            
            if (active.provider === 'aihorde') {
                active.smartRouting = sr.getElementById('horde-smart-routing').checked;
                active.maxContext = parseInt(sr.getElementById('horde-context-len').value, 10) || 4096;
                active.maxGen = parseInt(sr.getElementById('horde-gen-len').value, 10) || 512;
            }
    ...
    async initActiveProfile(forceFetch = false) {
        ...
        const emptyMsg = this.shadowRoot.getElementById('model-config-empty');
        ...
        const hordeSettings = this.shadowRoot.getElementById('aihorde-settings');
        const select = this.shadowRoot.getElementById('set-model');
        const manualSettings = this.shadowRoot.getElementById('horde-manual-settings');
        const smartToggle = this.shadowRoot.getElementById('horde-smart-routing');
        ...
        if (active.provider === 'aihorde') {
            ...
            smartToggle.checked = active.smartRouting !== false; // Default true
            manualSettings.style.display = smartToggle.checked ? 'none' : 'block';
            
            smartToggle.onchange = (e) => {
                manualSettings.style.display = e.target.checked ? 'none' : 'block';
            };

            this.shadowRoot.getElementById('horde-context-len').value = active.maxContext || 4096;
            this.shadowRoot.getElementById('horde-gen-len').value = active.maxGen || 512;
        } else {
        ...
        const activeModelsArray = active.model ? active.model.split(',') : [];
        select.innerHTML = this.availableModels.map(m =>
            `<option value="${m}" ${activeModelsArray.includes(m) ? 'selected' : ''}>${m}</option>`
        ).join('');
